...
def test_unpack():
    ...
    tile = Tile.Tile(TEST_ADDR, bytearray.fromhex(test_data))
    ...
    tile2 = Tile.Tile(TEST_ADDR, bytearray.fromhex(TEST_DATA), 8)
    assert bytearray(tile2.data).hex() == '0f0f0f0f00080605' * 8

def test_interleave_subtiles():
    test_data = 'F1' * 32 + 'F2' * 32 + 'F3' * 32 + 'F4' * 32

    interleaved = Tile._interleave_subtiles(bytearray.fromhex(test_data))
...
def test_deinterleave_subtiles():
    ...
    interleaved = Tile._interleave_subtiles(bytearray.fromhex(test_data))
    deinterleaved = Tile._deinterleave_subtiles(interleaved)
...
def test_pack():
    ...
    tile = Tile.Tile(TEST_ADDR, bytearray.fromhex(TEST_DATA16))
